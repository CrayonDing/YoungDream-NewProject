...
//会一直保持单线程连接
   ...
   $this->TestSQL();
   $this->getLogger()->info(TextFormat::GREEN."Z主线程:MySQL联通性可用.正在更新玩家异常数据.....");
...
public function onPreJoin(PlayerPreLoginEvent $e){
    ...
    $task=new GetAllTask($this->DB,$pn,"AccountPE","FinishGetAll");
 	 $this->getServer()->getScheduler()->scheduleAsyncTask($task);
       ...
       $e->getPlayer()->kick("CPA Servers Account System:(尝试更换ID /Try to change ID)\n同ID早已登录 / Already logined.");
    }
}
...
public function onChat(PlayerChatEvent $e){
   $pn=$e->getPlayer()->getName();
    ...
    $e->setCancelled();
    $m=array(
"cn"=>TextFormart::YELLOW."疑似误输密码".TextFormat::DARK_GRAY."\nCPA Servers Account System",
"en"=>TextFormart::YELLOW."Do not type your password again".TextFormat::DARK_GRAY."\nCPA Servers Account System"
);
    $this->SendMessage($e->getPlayer(),$m);
    }
    }
}
...
$t=0;
         ...
         $m=$m."已经在".$v["ol"]."登录,以踢出";
         $this->getServer()->getPlayer($p)->kick("CPA Servers Account System:(尝试更换ID /Try to change ID)\n已经在其他服务器登录 / Already logined in another server.
");
         }else{//正常登录         
if($this->isReg($p)){
//已注册
